    ...
    if (ConsoleUART->int_fl & MXC_F_UART_INT_FL_RX_FIFO_LVL) {
        ...
        while ((ConsoleUART->stat &
                ...
                if (!strcmp(rxBuf, "I"))
                {
                	...
                	unsigned int byte_ret = MXC_UART_ReadRXFIFO(ConsoleUART, receive_buffer, Max_byte_len);
                ...
                if (!strcmp(rxBuf, "B"))
				{
						unsigned int byte_ret1 = MXC_UART_ReadRXFIFO(ConsoleUART, receive_buffer1, Max_byte_len);
						crRecv = true;
                ...
                if (!strcmp(rxBuf, "G"))
				{
                	...
                	MAX77962_Charger_initial_settings(Term_voltage_rec,Chg_inp_curr_lmt,Fast_chg_curr_lsb,Fast_chg_curr_msb);
				}
                ...
                if (!strcmp(rxBuf, "J"))
				{
                	...
                	uint8_t PPS_Volt_LSB = receive_buffer6[2];
                	uint8_t PPS_Volt_MSB = receive_buffer6[3];
                	uint8_t PPS_curr = receive_buffer6[4];

                	uint16_t PPS_Volt_comb = (0x0FFF & (PPS_Volt_MSB << 8 | PPS_Volt_LSB))*20;
                	uint16_t PPS_curr_scaled = PPS_curr * 50;

                	Enable_PPS_Mode(Enable_PPS, 0x00FA, 0x1E);
                	MXC_Delay(500000);   //Delay is important. otherwise won't work
                	Set_PPS_Voltage_Current(Selected_APDO,PPS_Volt_comb, PPS_curr_scaled);  //default voltage: 5V(0x00FA), default current: 1500mA (0x1E)
				}
